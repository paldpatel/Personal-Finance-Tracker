...
var AuthService = class _AuthService {
  ...
  register(email, password, firstName, lastName) {
    return this.http.post(`${API}/auth/register`, { email, password, firstName, lastName }).pipe(tap((res) => this.persistSession(res)));
  }
  login(email, password) {
    return this.http.post(`${API}/auth/login`, { email, password }).pipe(tap((res) => this.persistSession(res)));
  }
  refreshToken() {
    const refreshToken = localStorage.getItem("refreshToken");
    return this.http.post(`${API}/auth/refresh`, { refreshToken }).pipe(tap((res) => {
...
//# sourceMappingURL=chunk-GGUZTU6B.js.map
